...
def heatmaps_from_one_sample(true : np.ndarray, pred : np.ndarray, one_scale = True, display = True):
    ...
    if not one_scale :
        ...
        cbar0.set_label("displacement (mm)")

    cbar1 = fig.colorbar(im1, ax=axs[1])
    cbar1.set_label("displacement (mm)")

    im2 = axs[2].imshow(diff, cmap='Oranges')
    axs[2].set_title("Absolute Error (mm)")
    axs[2].axis('off')
...
def results_from_csv_v2(csv_path) :

    df = pd.read_csv(csv_path, header=0, index_col=None, sep=',', dtype=str)
    df = df[df['Frame'] != 'Increment      0: Base State']

    dfs_by_frame = [group for _, group in df.groupby('Frame')]

    data_per_mode = {}

    for frame in dfs_by_frame :
        info = frame['Frame'].iloc[0]
        mode = int(info.split(":")[0].split("Mode")[-1])
        value = float(info.split("Value =")[-1].split("Freq =")[0])
        frequ = float(info.split("Freq =")[-1].split("(cycles/time)")[0])

        info_dict = {"value" : value, "frequ" : frequ}
        U_U1 = frame["           U-U1"].to_numpy()
        U_U2 = frame["           U-U2"].to_numpy()
        U_U3 = frame["           U-U3"].to_numpy()
        
        data = np.stack((U_U1, U_U2, U_U3)).transpose()

        data_per_mode[str(mode)] = [info_dict, data]
...
if __name__ == "__main__":
    ...
    results_1_path = os.path.join("data\\raw\\20250428175523", f"results\\0.csv")
    results_2_path = os.path.join("data\\raw\\20250512105014", f"results\\0.csv")

    results1 =  results_from_csv(results_1_path)
    results2 =  results_from_csv_v2(results_2_path)
