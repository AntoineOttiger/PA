...
mtx_path = os.path.join(grid_path, "mtx")

sim_file = "sim_v2.py"
global_sim_py = os.path.join(run_sims_path, sim_file)
temp_path=r"C:\temp"
sim_path=os.path.join(temp_path, sim_file)
...
for index, combo in enumerate(combinations):
    ...
    if os.path.isfile(results_path):
        ...
        #içi on sauve tout les fichiers de la v2
        this_sim_mtx_path = os.path.join(mtx_path, str(index))
        os.makedirs(this_sim_mtx_path)
        shutil.move(os.path.join(temp_path,"matrix_clean.inp"), os.path.join(this_sim_mtx_path, "matrix_clean.inp"))
        shutil.copy(os.path.join(temp_path,"matrix_clean_mod.inp"), os.path.join(this_sim_mtx_path, "matrix_clean_mod.inp"))
        shutil.move(os.path.join(temp_path,"matrix_clean_mod_MASS1.mtx"), os.path.join(this_sim_mtx_path, "matrix_clean_mod_MASS1.mtx"))
        shutil.move(os.path.join(temp_path,"matrix_clean_mod_STIF1.mtx"), os.path.join(this_sim_mtx_path, "matrix_clean_mod_STIF1.mtx"))

        # on garde pour retrocompatibilité v1
        inp_path = os.path.join(positions_path, f"{index}.inp")
        shutil.move(os.path.join(temp_path, "matrix_clean_mod.inp"), inp_path )
